...
export function applyWaterWaveShader(
  ...
  // Both slots go through the helper so this cannot regress to a replacement.
  // It previously assigned customProgramCacheKey and onBeforeCompile outright,
  // which was safe only because nothing else patches the ocean — an assumption
  // no type or test was holding in place.
  patchMaterial({
    material,
    cacheKey: 'aiboids-water-waves-v1',
    patch: (shader) => {
